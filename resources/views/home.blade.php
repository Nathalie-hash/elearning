...
@section('content')
    ...
    @hasanyrole('administrateur|etudiant|enseignant')
        @foreach($user->niveaux as $niveau)
            ...
            <h4 style="font-size:18px;padding-bottom:10px;font-weight:bold;color:#7d2ae7;border-bottom:1px solid #7d2ae7 !important"><a href="/niveau/{{ $niveau->id }}" class="btn btn-primary p-5" style="text-decoration: none"> Niveau {{$niveau->nom}}</a></h4>
